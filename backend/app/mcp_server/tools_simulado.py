...
from app.services import questoes as acervo
...
def detalhar_questao(
    ...
    with _sessao() as (db, ident):
        return acervo.detalhar_questao(db, ident, questao)
...
async def editar_questao(
    ...
    **Antes de chamar, mostre ao professor no chat como vai ficar — use
    ...
    return await _service(
        ident, acervo.editar_questao, questao,
...
def remover_questao(questao: Annotated[int, Field(description="Id da questão")]) -> dict:
    ...
    **Antes de chamar, mostre ao professor no chat qual questão sai e espere o
    ...
    """
    with _sessao() as (db, ident):
        return acervo.remover_questao(db, ident, questao)
...
async def editar_simulado(
    ...
    * encerrado: só o título.
    ...
    ident = identidade_da_sessao()
    return await _service(
        ident, simulados.editar_simulado, simulado,
        titulo=titulo, abre_em=abre_em, fecha_em=fecha_em, duracao_minutos=duracao_minutos,
        turmas=turmas,
