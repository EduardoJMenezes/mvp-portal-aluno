...
def test_o_portal_estatico_e_o_ultimo_a_ser_consultado() -> None:
    """A regra que a versão anterior violou.
...
def test_a_api_do_portal_continua_atras_do_mcp() -> None:
    """O MCP passou a hospedar o FastAPI; o REST não pode ter sumido no caminho."""
    cliente = TestClient(main.app, raise_server_exceptions=False)

    # Sem banco nesta máquina a saúde responde 503 — o que importa aqui é que
    # a requisição chega ao handler do FastAPI em vez de se perder no MCP.
    assert cliente.get("/api/saude").status_code in (200, 503)
...
def test_login_mapeado_vira_o_operador_da_plataforma(db, com_oauth) -> None:
...
def test_email_publico_do_github_vale_quando_bate_com_um_operador(db, com_oauth) -> None:
    _operador(db, "helena@escola.demo")

    assert _operador_do_github(["helena@escola.demo"])["email"] == "helena@escola.demo"


def test_login_desconhecido_nao_abre_sessao(db, com_oauth) -> None:
    _operador(db, "professor@escola.demo")

    assert _operador_do_github(["estranho", "estranho@exemplo.com"]) is None


def test_aluno_nao_entra_pelo_github_nem_estando_no_mapa(db, com_oauth, monkeypatch) -> None:
