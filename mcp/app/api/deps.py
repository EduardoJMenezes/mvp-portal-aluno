...
from app.security import hash_token, le_jwt
...
def usuario_atual(
    ...
    if not dados:
        ...
        return _pelo_token_do_mcp(db, valor)
...
def _pelo_token_do_mcp(db: Session, valor: str) -> Identidade:
    ...
    A conferência é aqui, no banco, e não pela API em Java: o portal tem a
    tabela na mão. Quem pergunta ao Java é o adaptador MCP, que não tem — e é
    por isso que as duas conferências existem, com o mesmo SHA-256 dos dois
    lados. Ver `mcp_server/auth.py`.
    """
    registro = db.scalar(
        select(TokenMCP).where(
            TokenMCP.token_hash == hash_token(valor), TokenMCP.revogado.is_(False)
        )
    )
    if registro is None or registro.usuario.papel not in Papel.OPERADORES:
